...
### sub_category (서브카테고리) - 반드시 아래 목록에서 선택
category 안에서 곡의 세부 유형을 구분한다.

- 관심: 눈에 띄는 존재 / 의식하는 시작
- 짝사랑: 바라보는 짝사랑 / 고백 전 짝사랑 / 이루어질 수 없는 짝사랑
- 썸: 설레는 밀당 / 불안한 밀당 / 확인하고 싶은 마음
- 사랑: 달콤한 연애 / 깊어지는 사랑 / 약속과 맹세
- 권태기: 식어가는 감정 / 무감각한 일상
- 갈등: 오해와 다툼 / 상처 주고받기
- 이별: 그리운 이별 / 후련한 이별 / 일방적 이별 / 아픈 이별 / 아름다운 이별
- 자기자신: 자존감 회복 / 성장과 다짐 / 자기 위로
  주의: 이별/연애 맥락이 조금이라도 있으면 자기자신이 아님. "쿨하게 보내줌", "너 없이도 잘 살아" 등은 이별이다.
- 일상: 친구 / 가족 / 일상의 감정
- 기타: 기타
...
### tags (표준 태그) - 반드시 아래 사전에서만 선택 (2~4개)

[감정 태그] 그리움, 미련, 후회, 원망, 분노, 후련, 체념, 외로움, 설렘, 두근거림, 행복, 기대, 불안, 혼란, 상처, 위로
[상황 태그] 고백, 밀당, 재회, 기다림, 추억, 잊기, 놓아줌, 매달림, 거절, 배신, 성장, 다짐, 여행, 계절, 일상, 파티
[관계 태그] 연인, 짝사랑, 썸, 친구, 가족, 자기자신
...
{{
  ...
  "sub_category": "서브카테고리 (SYSTEM_PROMPT의 목록에서 선택)",
...
def _validate_result(result: dict) -> dict:
    ...
    STANDARD_TAGS = {
        "그리움", "미련", "후회", "원망", "분노", "후련", "체념", "외로움",
        "설렘", "두근거림", "행복", "기대", "불안", "혼란", "상처", "위로",
        "고백", "밀당", "재회", "기다림", "추억", "잊기", "놓아줌", "매달림",
        "거절", "배신", "성장", "다짐", "여행", "계절", "일상", "파티",
        "연인", "짝사랑", "썸", "친구", "가족", "자기자신",
    }
    result["tags"] = [t for t in result["tags"] if t in STANDARD_TAGS]
